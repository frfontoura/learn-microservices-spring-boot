...
public class MultiplicationResultAttemptControllerTest {
    ...
    public void getUserStatsTest() throws Exception {
        ...
        final List<MultiplicationResultAttempt> recentAttempts = Lists.newArrayList(attempt, attempt);

        given(multiplicationService.getStatsForUser(user.getAlias())).willReturn(recentAttempts);

        final MockHttpServletResponse response = mvc.perform(get("/results")
                .param("alias", user.getAlias()))
                .andReturn()
                .getResponse();
